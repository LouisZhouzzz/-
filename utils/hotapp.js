...
var hotAppHost = "https://wxapi.hotapp.cn",
  ...
  uploadType = 0,
  debugarr = [],
  hotAppDebug = !1; function onError(a, f, b) {
    var c = getSystemInfo(),
      d = getUserInfo(),
      e = hotAppHost + "/api/error"; a = { appkey: getHotAppKey(), system_info: c, user_info: d, version: f, msg: a }; http(e, a, b);
  } function getVersion() {
...
function log(a) {
  ...
  var a = wx.getStorageSync("key"); if (a) return a; a = userInfo.avatarUrl ? hotAppKey + userInfo.avatarUrl : hotAppKey + parseInt(9E4 * Math.random() + 1E4, 10); a = hex_md5(a); wx.setStorageSync("key", a); return a;
} function getFakeOpenID() {
  var a = getOpenID(); return a ? a : getLocalKey();
}
function getPrefix(a) {
  return a + "_" + getFakeOpenID();
} function genPrimaryKey(a) {
  var f = Date.parse(new Date()); return a + "_" + getFakeOpenID() + "_" + 1E3 * f;
} function replaceOpenIdKey(a, f) {
  if (!getOpenID()) return "function" == typeof f && f(!1); var b = a.replace("_" + getLocalKey() + "_", "_" + getOpenID() + "_"); return "function" == typeof f && f(b);
}
function login(a) {
  if (!hotAppKey) return "function" == typeof a && a(getFakeOpenID()); var f = getOpenID(); if (f) return "function" == typeof a && a(f); wx.login({
    success: function success(b) {
      if (b.code) wx.request({
        url: hotAppHost + "/data/wechat/login", data: { hotAppKey: getHotAppKey(), code: b.code }, method: "POST", success: function success(b) {
      ...
      }); else return "function" == typeof a && a(getFakeOpenID());
    }, fail: function fail() {
      return "function" == typeof a && a(getFakeOpenID());
    }
  });
...
function init(a) {
  if (!a) return log("appkey\u4E0D\u80FD\u4E3A\u7A7A"), "function" == typeof cb && cb(!1); if (hotAppKey) return log("\u5DF2\u7ECF\u521D\u59CB\u5316\u8FC7\u4E86"), "function" == typeof cb && cb(!1); hotAppKey = a; wx.login({
    success: function success(a) {
      getOpenID() ? wx.getUserInfo({
        success: function success(a) {
          userInfo = a.userInfo; sendLaunch();
        }
        ...
        url: hotAppHost + "/data/wechat/login", data: { hotAppKey: getHotAppKey(), code: a.code },
        method: "POST", success: function success(a) {
          (a = a.data.openid) && setOpenID(a); wx.getUserInfo({
            success: function success(a) {
              userInfo = a.userInfo; sendLaunch();
            }
          });
...
function sendLaunch() {
  wx.request({
    url: hotAppHost + "/data/wechat/launch", data: { hotAppKey: getHotAppKey(), openId: getOpenID(), hotAppUUID: getHotAppUUID(), userInfo: getUserInfo(), systemInfo: getSystemInfo(), phoneTime: Date.parse(new Date()) / 1E3, hotAppVersion: hotAppVersion }, method: "POST", success: function success(a) {
      0 == uploadType && (uploadType = a.data.upload_type); if (0 != uploadType) {
        var f = wx.getStorageSync("hotAppEvent") || []; 0 != f.length && wx.request({
          url: hotAppHost + "/data/wechat/event", data: {
            hotAppKey: getHotAppKey(), openId: wetChatOpenId,
            hotAppUUID: getHotAppUUID(), eventArray: f
          }, method: "POST", success: function success(a) {
            log(wx.getStorageSync("hotAppEvent") || []); log(a.data); try {
              wx.removeStorageSync("hotAppEvent");
            } catch (c) {
              log(c);
            }
          }, fail: function fail() {
            log("send event fail"); wx.setStorageSync("hotAppEvent", f);
          }
        });
      }
...
function onEvent(a, f) {
  f = void 0 === f ? "" : f; if ("" == hotAppKey) log("hotappkey is empty"); else {
    var b = wx.getStorageSync("hotAppEvent") || [],
      c = { eventId: a, eventValue: f, phoneTime: Date.parse(new Date()) / 1E3 }; b.push(c); 0 != uploadType ? wx.setStorageSync("hotAppEvent", b) : wx.request({
        url: hotAppHost + "/data/wechat/event", data: { hotAppKey: getHotAppKey(), openId: getOpenID(), hotAppUUID: getHotAppUUID(), eventArray: b, hotAppVersion: hotAppVersion }, method: "POST", success: function success(a) {
          log(wx.getStorageSync("hotAppEvent") || []); try {
            wx.removeStorageSync("hotAppEvent");
          } catch (b) {
            log(b);
          }
        ...
        fail: function fail() {
          log("send event fail"); wx.setStorageSync("hotAppEvent", b);
        }
      });
  }
} function getHotAppUUID() {
  if ("" == hotAppKey) log("hotappkey is empty"); else {
    if ("" == hotAppUUID) if ("" == wx.getStorageSync(hotAppUUIDCache)) {
      if ("" == userInfo) return log("userInfo is null"), ""; hotAppUUID = "" == userInfo.avatarUrl ? hex_md5(hotAppKey + userInfo.nickName) : hex_md5(hotAppKey + userInfo.avatarUrl); wx.setStorageSync(hotAppUUIDCache, hotAppUUID);
    } else hotAppUUID = wx.getStorageSync(hotAppUUIDCache); return hotAppUUID;
  }
}
function getHotAppKey() {
  return hotAppKey;
} function clearData() {
  hotAppUUID = ""; wx.clearStorage();
...
function core_md5(a, f) {
  a[f >> 5] |= 128 << f % 32; a[(f + 64 >>> 9 << 4) + 14] = f; for (var b = 1732584193, c = -271733879, d = -1732584194, e = 271733878, g = 0; g < a.length; g += 16) {
    var h = b,
      k = c,
      l = d,
      m = e,
...
function md5_hh(a, f, b, c, d, e, g) {
  ...
  var b = str2binl(a); 16 < b.length && (b = core_md5(b, a.length * chrsz)); for (var c = Array(16), d = Array(16), e = 0; 16 > e; e++) {
    c[e] = b[e] ^ 909522486, d[e] = b[e] ^ 1549556828;
  } b = core_md5(c.concat(str2binl(f)), 512 + f.length * chrsz); return core_md5(d.concat(b), 640);
} function safe_add(a, f) {
  var b = (a & 65535) + (f & 65535); return (a >> 16) + (f >> 16) + (b >> 16) << 16 | b & 65535;
}
...
function binl2b64(a) {
  ...
  http(hotAppHost + "/api/get", { appkey: hotAppKey, key: a }, f);
}
function post(a, f, b) {
  http(hotAppHost + "/api/post", { appkey: hotAppKey, key: a, value: f }, b);
} function del(a, f) {
  var b = hotAppHost + "/api/delete",
    c = { appkey: getHotAppKey(), key: a }; http(b, c, f);
} function feedback(a, f, b, c) {
  var d = getSystemInfo(),
    e = getUserInfo(); if (!e) return log("userinfo is empty"), "function" == typeof c && c(!1); var g = hotAppHost + "/api/feedback"; a = { appkey: getHotAppKey(), content: a, openid: getOpenID() ? getOpenID() : getFakeOpenID(), content_type: f, contract_info: b, system_info: d, user_info: e }; http(g, a, c);
}
function uploadFeedbackImage(a) {
  wx.chooseImage({
    success: function success(f) {
      console.log(f); wx.uploadFile({
        url: hotAppHost + "/api/feedback/image/upload", filePath: f.tempFilePaths[0], name: "file", formData: { appkey: getHotAppKey() }, success: function success(b) {
          b = JSON.parse(b.data); return 0 == b.ret ? "function" == typeof a && a(b.image_url) : "function" == typeof a && a(!1);
...
function http(a, f, b) {
  wx.request({
    url: a, data: f, method: "POST", header: { "content-type": "application/json" }, success: function success(a) {
...
function request(a) {
  0 == a.useProxy ? wx.request({
    url: a.url, data: a.data, header: a.header, method: a.method, success: function success(f) {
      a.success(f);
    }, fail: function fail(f) {
      a.fail(f);
    ...
    url: hotAppHost + "/proxy/?appkey=" + hotAppKey + "&url=" + a.url, data: a.data, header: a.header, method: a.method, success: function success(f) {
      a.success(f);
    }, fail: function fail(f) {
      a.fail(f);
    }, complete: function complete(f) {
...
function onLoad(a, f) {
  "object" == (typeof a === "undefined" ? "undefined" : _typeof(a)) && a.__route__ ? "object" != (typeof f === "undefined" ? "undefined" : _typeof(f)) || 0 == Object.getOwnPropertyNames(f).length ? log("param error") : "" == hotAppKey ? log("hotapp key is empty") : login(function (b) {
    var c = hotAppHost + "/data/wechat/param"; b = { hotAppKey: hotAppKey, page: a.__route__, openId: b, hotAppUUID: getHotAppUUID(), paraInfo: f }; http(c, b);
  }) : log("param error");
}
module.exports = { init: init, onEvent: onEvent, setEventUploadType: setEventUploadType, clearData: clearData, wxlogin: login, getFakeOpenID: getFakeOpenID, getOpenID: getOpenID, getPrefix: getPrefix, genPrimaryKey: genPrimaryKey, replaceOpenIdKey: replaceOpenIdKey, searchkey: searchkey, get: get, post: post, del: del, request: request, getVersion: getVersion, setDebug: setDebug, feedback: feedback, uploadFeedbackImage: uploadFeedbackImage, onError: onError, onLoad: onLoad };